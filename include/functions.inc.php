...
function formatFiles($row) {
	...
	*/
	//$record_datetime = DateTime::createFromFormat('Y-m-d G:i:s', $row['calldate']);
	//$recorded_file = date_format($record_datetime, 'Y/m/d') .'_'. $row['src'] .'-'. $row['dst'];
	/* ============================================================================ */
	...
	*/
	...
	if (file_exists("$system_monitor_dir/$recorded_file.$system_audio_format")) {
		echo "    <td class=\"record_col\"><a href=\"download.php?audio=$recorded_file.$system_audio_format\" title=\"Прослушать запись\"><img src=\"templates/images/sound.png\" alt=\"Запись звонка\" /></a><img class=\"record_preview_icon\" src=\"\" onclick=\"audioPreview(this,false);\"/><div class=\"record_preview_lt\"></td>\n";
		//echo "    <td class=\"record_col\"><audio controls><a href=\"$recorded_file.$system_audio_format\" title=\"Listen to call recording\"><img src=\"templates/images/sound.png\" alt=\"Call recording\" /></a></audio></td>\n";
		...
		echo "    <td class=\"record_col\"><a href=\"download.php?arch=$recorded_file.$system_audio_format.$system_arch_audio_format\" title=\"Загрузить архивом\"><img src=\"templates/images/compressed.png\" alt=\"Запись звонка\" /></a></td>\n";
	} elseif (file_exists("$system_fax_archive_dir/$recorded_file.tif")) {
		echo "    <td class=\"record_col\"><a href=\"download.php?fax=$recorded_file.tif\" title=\"Просмотр FAX\"><img src=\"templates/images/text.png\" alt=\"FAX\" /></a></td>\n";
	} elseif (file_exists("$system_monitor_dir/$recorded_file")) {
		echo "    <td class=\"record_col\"><a href=\"download.php?audio=$recorded_file\" title=\"Прослушать запись\"><img src=\"templates/images/sound.png\" alt=\"Запись звонка\" /></a><img class=\"record_preview_icon\" src=\"\" onclick=\"audioPreview(this,false);\"/><div class=\"record_preview_lt\"></td>\n";
		//echo "    <td class=\"record_col\"><audio controls><a href=\"$recorded_file\" title=\"Listen to call recording\"><img src=\"templates/images/sound.png\" alt=\"Call recording\" /></a></audio></td>\n";
	} else {
...
function formatCallDate($calldate,$uniqueid) {
	echo "    <td class=\"record_col\"><abbr title=\"Уникальный ID: $uniqueid\">$calldate</abbr></td>\n";
}

function formatChannel($channel) {
	$trunk_name = preg_replace('/(.*)-[^-]+$/','$1',$channel);
	echo "    <td class=\"record_col\"><abbr title=\"Канал: $channel\">$trunk_name</abbr></td>\n";
}
...
function formatSrc($src,$clid) {
	if (empty($src)) {
		echo "    <td class=\"record_col\">Неизвестно</td>\n";
	} else {
		$src = htmlspecialchars($src);
		$clid = htmlspecialchars($clid);
		echo "    <td class=\"record_col\"><abbr title=\"ID звонящего: $clid\">$src</abbr></td>\n";
	}
}

function formatApp($app, $lastdata) {
	if($app == 'Dial'){$app_var_L = 'Руч. набор';}
	if($app == 'Congestion'){$app_var_L = 'Сброс звонка';}
	if($app == 'Queue'){$app_var_L = 'Очередь';}
	if($app == 'Playback'){$app_var_L = 'Аудиозапись';}
	if($app == 'Busy'){$app_var_L = 'Занято';}
	if($app == 'Hangup'){$app_var_L = 'Завершение звонка';}
	echo "    <td class=\"record_col\"><abbr title=\"ПО: $app($lastdata)\">$app_var_L ($app)</abbr></td>\n";
}
...
function formatDst($dst, $dcontext) {
	...
	if (strlen($dst) == 11 and strlen($rev_lookup_url) > 0 ) {
		...
		echo "    <td class=\"record_col\"><abbr title=\"Контекст назначения: $dcontext\"><a href=\"$rev\" target=\"reverse\">$dst</a></abbr></td>\n";
	} else {
		echo "    <td class=\"record_col\"><abbr title=\"Контекст назначения: $dcontext\">$dst</abbr></td>\n";
	}
}
...
function formatDisposition($disposition, $amaflags) {
	...
	//if($disposition == 'ANSWERED'){$disposition_var_L = 'З ВІДПОВІДДЮ';}
	//if($disposition == 'FAILED'){$disposition_var_L = 'ПОМИЛКА';}
	//if($disposition == 'BUSY'){$disposition_var_L = 'ЗАЙНЯТО';}
	//if($disposition == 'NO ANSWER'){$disposition_var_L = 'НЕМАЄ ВІДПОВІДІ';}

	if($disposition == 'ANSWERED'){$disposition_var_L = 'ОТВЕЧЕННЫЙ';}
	if($disposition == 'FAILED'){$disposition_var_L = 'ОШИБКА';}
	if($disposition == 'BUSY'){$disposition_var_L = 'ЗАНЯТО';}
	if($disposition == 'NO ANSWER'){$disposition_var_L = 'НЕТ ОТВЕТА';}

	if ( $disposition == 'ANSWERED' ) {
		echo "    <td class=\"record_col\"><abbr title=\"AMA Flag: $amaflags\"><span class='green'>$disposition_var_L</span></abbr></td>\n";
	} else {
		echo "    <td class=\"record_col\"><abbr title=\"AMA Flag: $amaflags\"><span class='red'>$disposition_var_L</span></abbr></td>\n";
	}
}
...
function formatDuration($duration, $billsec) {
	...
	echo "    <td class=\"record_col\"><abbr title=\"Длительность: $billduration\">$duration</abbr></td>\n";
}
